...
from utils import TimezoneChoices, View, Select, format_schedule, format_task
...
class MyCogs(commands.Cog):
    ...
    async def on_resumed(self):
        ...
        if self.worker.is_suspended:
            ...
            print(self.worker._sleeping.result(), self.worker.is_running)
    ...
    async def task_list(self, inter: disnake.AppCmdInter):
        ...
        async for i in DATABASE.find({"user": inter.author.id}).sort("posix_time", 1):
            ...
            if i["once"]:
                tasks += format_task(i["time"], i["details"], i["completed"])
                continue
            ...
            schedule[wd] += format_schedule(i["time"].time(), i["details"])
        ...
        emb.add_field(
            name="Tasks",
            value=tasks or "You have no tasks set.",
            inline=False,
        )
        await inter.response.send_message(
            embed=emb, ephemeral=True, view=View(self.worker)
        )
...
bot = commands.InteractionBot(
    activity=disnake.Activity(name="/help", type=disnake.ActivityType.watching)
)
